...
<div class="container-lg">
    <div class="admin-header">
        ...
        <div class="d-flex gap-2">
            <a href="/" class="btn btn-secondary btn-small">← Back to App</a>
            <a href="/admin?logout=1" class="btn btn-small" style="background:var(--surface2);color:var(--text)">Log Out</a>
        </div>
    </div>
            ...
            <?php else: ?>
                <table class="table table-striped table-hover results-table w-100" id="questions-table">
                    ...
                    <?php foreach ($questions as $q):
                        ...
                        $correct = $opts[$q['correct_index']] ?? '';
                    ?>
                        <tr class="question-row" id="qrow-<?= $q['id'] ?>">
                            <td style="color:var(--text-muted)"><?= $q['id'] ?></td>
                            <td>
                                <div><?= htmlspecialchars($q['question_text']) ?></div>
                                <div class="option-preview mt-1">
                                    <?php foreach ($opts as $i => $opt): ?>
                                        <?php if ($i === (int)$q['correct_index']): ?>
                                            <span class="correct-badge">✓ <?= htmlspecialchars($opt) ?></span>
                                        <?php else: ?>
                                            <span class="me-2"><?= htmlspecialchars($opt) ?></span>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </div>
                            </td>
                            <td><span class="badge" style="background:var(--primary)"><?= $q['category'] ?></span></td>
                            <td style="color:var(--text-muted)"><?= $q['difficulty'] ?></td>
                            <td>
                                <button class="btn btn-small delete-btn" style="background:var(--danger);color:#fff" data-id="<?= $q['id'] ?>">Delete</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
...
$('#add-form').on('submit', function (e) {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            const err = xhr.responseJSON?.error || 'Failed to add question.';
            $('#form-error').text(err);
...
$(document).on('click', '.delete-btn', function () {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            showToast(xhr.responseJSON?.error || 'Could not delete.', false);
        }
    });
});
</script>
